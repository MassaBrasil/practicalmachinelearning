...
#loads of package, data, checks/explorations
...
# Compare columns if are equal in both datasets
```{r compare columns}
...
# Split training dataset in two : 1 for training, 2 for validation 
...
- random forest (rf)
- combination of two most accurate models 

## Model evaluation 


### Cross validation
Cross validation was specified Using trainControl function, with fold number set to 34 to achieve sample size between not so few chunks and not many but with in small size.
...
#Combining RF and GBM - modeling and predict
```{r comb RF GBM}
comb_DF <- data.frame(predict_rf,predict_gbm,classe=pml_train_2$classe)
comb_ModFit <- train(classe~.,method="gam",data=comb_DF, verbose=FALSE)
pred_DF <- predict(comb_ModFit,comb_DF)
```

```{r accuracy - combination}
cm_comb <- confusionMatrix(pred_DF, pml_train_2$classe)

Sys.time()
```


# Conclusion - Predicting Results on the Test Data

The accuracy found on 4 regression modeling methods seen above were:

Random Forest : `r  cm_rf$overall[1]`
Decision Tree : `r  cm_dt$overall[1]`
GBM           : `r  cm_gbm$overall[1]`
combine rf+GBM: `r  cm_comb$overall[1]`
...
For the model chosen, expected out-of-sample error found was 1.0-`r  cm_rf$overall[1]` = `r 1.0 - cm_rf$overall[1]` %.
